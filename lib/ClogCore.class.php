...
class ClogCore{
    ...
    protected function userAgent(){
        return "{$this->client_name}-{$this->client_version} (clog_core-{$this->core_version})";
    }
    ...
    protected function getConfigData($config_file_or_options){
        ...
        $this->loadConfigFile('clog.ini');
        $this->loadConfigFile('clog.json');

        $this->loadFromEnv('CLOG');

        $this->loadConfigFile(self::homeDir() . '.clog.ini');
        $this->loadConfigFile(self::homeDir() . '.clog.json');
    ...
    protected function loadConfigFile($file){
        ...
        if (!empty($data['clog'])) $this->loadFromHash($data['clog']);
    ...
    protected function apiCall($type, $url, $params = array(), $raw_post_data = null){
        ...
        //$url = "{$this->protocol}://{$this->host}:{$this->port}/api/v{$this->api_version}$url";
        $url = "{$this->protocol}://{$this->host}:{$this->port}$url";
